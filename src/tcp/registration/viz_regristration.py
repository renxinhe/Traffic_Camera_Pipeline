...
class VizRegristration():
    ...
    def visualize_trajectory_dots(self, trajectories, filter_class=None, plot_traffic_images=False, video_name=None):
        ...
        '''
        assert filter_class is None or filter_class == 'car' or filter_class == 'pedestrian',\
            'Invalid filter_class: should be car or pedestrian.'
        ...
        max_time_horizon = -float('inf')
        for trajectory in trajectories:
            max_time_horizon = max(max_time_horizon, trajectory.get_last_state_timestep())

        for t in range(min(max_time_horizon, self.config.vz_time_horizon)):
            for traj_index, traj in enumerate(trajectories):
                if filter_class is not None and traj.class_label != filter_class:
                    continue
